...
// web, and apps/customer-mobile's theme) — single source of truth.
export const colors = {
  bg: "#0A0A0C",
  surface: "#1C1C21",
  border: "#2A2A30",
    ...
    <SafeAreaView edges={["top", "left", "right"]} style={s.screen}>
      ...
      >
        <View style={s.header}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.brand}>RIDEGRID / VENDOR</Text>
...
export function BookingCard({ booking: b }: { booking: Booking }) {
  return (
    <Pressable
      onPress={() =>
        router.push({ pathname: "/booking", params: { id: b.id } })
      }
    >
      <Card>
        <View style={s.row}>
        ...
        <Label muted>
          ...
          {b.pricingPackage?.packageType || b.tripType}
        </Label>
        <Label>
          {b.vehicle?.registrationNumber} · {name(b.driver)}
        </Label>
        {b.vendorEarning != null && (
          <Label>Vendor earning {money(b.vendorEarning)}</Label>
        )}
      </Card>
    </Pressable>
  );
