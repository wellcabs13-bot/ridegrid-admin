...
async function raw(path: string, init: RequestInit = {}) {
  if (
    !/^https:\/\//.test(baseURL) &&
    !(typeof __DEV__ !== "undefined" && __DEV__ && /^http:\/\//.test(baseURL))
  )
    throw new Error(
      "This app cannot connect to RideGrid. Please contact support.",
    );
...
async function refreshSession() {
  ...
  if (!response.ok || !validSession(body.data)) {
    if ((response.status === 401 || response.ok) && before === generation)
