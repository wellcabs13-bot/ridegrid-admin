...
export function Providers({ children }: React.PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    const unsub = onSession((s) => {
      if (previousId && previousId !== s?.user.id)
        void clearOffline(previousId).catch(() => {});
    ...
    void sessionStore
      ...
      .then(async (stored) => {
        ...
        if (stored) { const user = await api<User>("/api/auth/me").catch(() => null); if (user && user.role !== "DRIVER") await setSession(null); }
      })
      .catch(() => {
