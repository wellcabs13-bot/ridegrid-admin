...
vi.mock("../../apps/driver-mobile/src/storage/session", () => ({
  sessionStore: store,
  validSession: (s: any) =>
    !!s?.accessToken && !!s?.refreshToken && s?.user?.role === "DRIVER",
}));
const session = {
  accessToken: "expired",
...
describe("mobile authenticated transport", () => {
  it("rotates refresh tokens once for concurrent 401s and retries with the new access token", async () => {
    ...
    const api = await import("../../apps/driver-mobile/src/services/api");
