...
export function Hero({ title, eyebrow, description, media, links = [], breadcrumbs = [] }: { title: string; eyebrow: string; description?: string; media?: PublicMedia | null; links?: PublicLink[]; breadcrumbs?: PublicLink[] }) {
  return <section className={s.hero}><div className={s.container}><Breadcrumbs links={breadcrumbs} /><div className={s.heroGrid}><div><p className={s.eyebrow}>{eyebrow}</p><h1>{title}</h1>{description && <p className={s.heroDescription}>{description}</p>}<div className={s.actions}>{links.map((link, i) => <PublicAnchor key={i} link={link} className={i ? s.secondaryButton : s.button}>{link.label}<ArrowUpRight size={17} /></PublicAnchor>)}</div></div><div className={s.heroVisual}>{media ? <Image src={media.src} alt={media.alt} fill priority sizes="(max-width: 700px) 100vw, 48vw" unoptimized className="object-cover" /> : <div className={s.road} aria-hidden="true" />}<p className={s.visualCaption}>{media?.caption || "The everyday. The getaway. Your next journey."}</p></div></div></div></section>;
}
...
export function RelatedPages({ heading = "Explore more journeys", links }: { heading?: string; links: (PublicLink & { type?: string; description?: string })[] }) {
  ...
  return <section id="discover" className={s.section}><div className={s.container}><p className={s.eyebrow}>Discover RideGrid</p><h2>{heading}</h2><div className={s.grid}>{links.map((link, i) => <article key={`${link.href}-${i}`} className={s.card}>{link.type && <span className={s.tag}>{link.type}</span>}<h3><PublicAnchor link={link} /></h3>{link.description && <p>{link.description}</p>}<PublicAnchor link={link} className={s.cardLink}>Explore journey <span aria-hidden="true">↗</span></PublicAnchor></article>)}</div></div></section>;
}
