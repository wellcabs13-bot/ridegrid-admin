...
type Booking = {
  ...
  tripType?: string;
  trip?: { status: string; deletedAt: string | null; arrivedPickupAt: string | null; tripStartedAt: string | null; tripCompletedAt: string | null } | null;
  status: string;
...
export default function BookingsPage() {
  ...
  const loadBookings = useCallback(async (background = false) => {
    try {
      ...
      const rows: Booking[] = Array.isArray(result.data) ? result.data : [];
      setBookings(rows);
  ...
  useEffect(() => { void loadBookings(); const timer = setInterval(() => { if (document.visibilityState === "visible") void loadBookings(true); }, 30000); return () => clearInterval(timer); }, [loadBookings]);
  useEffect(() => {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          <button onClick={() => void loadBookings()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-60">
    ...
    {b.bookingSource === "CORPORATE"
      ...
      : b.bookingSource === "APP"
        ...
        : "Website"}
                    ...
                    <td className="px-5 py-5"><p className="font-bold text-slate-800">{customer||"—"}</p><p className="mt-1 text-xs text-slate-500">{b.customer?.user?.mobile||b.customer?.user?.email||"No contact"}</p></td>
                    ...
                    <td className="px-5 py-5"><span className={`inline-flex rounded-full border px-3 py-1.5 text-xs font-black ${bookingStatusClass(b.status)}`}>{statusLabel(b.status)}</span>{b.trip && !b.trip.deletedAt && !["CANCELLED", "TRIP_COMPLETED"].includes(b.status) && <p className="mt-2 text-xs font-bold text-blue-700">{statusLabel(b.trip.status)}</p>}</td>
                    <td className="px-5 py-5"><span className={`inline-flex rounded-full border px-3 py-1.5 text-xs font-black ${paymentStatusClass(tx?.paymentStatus||"PENDING")}`}>{statusLabel(tx?.paymentStatus||"PENDING")}</span><p className="mt-1 text-xs text-slate-400">{statusLabel(tx?.paymentMethod||"—")}</p></td>
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
    <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
      ...
      <div className="grid gap-5 p-6 md:grid-cols-2">
        ...
        <DetailCard title="Journey" icon={<MapPin size={18}/>}><p className="mb-3 font-bold text-blue-700">{statusLabel(["CANCELLED", "TRIP_COMPLETED"].includes(booking.status) ? booking.status : booking.trip && !booking.trip.deletedAt ? booking.trip.status : booking.status)}</p><p className="text-sm font-bold text-slate-800">Pickup: {booking.pickupLocation}</p><p className="mt-3 text-sm font-bold text-slate-800">Drop: {booking.dropLocation}</p><p className="mt-3 text-sm text-slate-500">{formatDate(booking.pickupDateTime)} at {formatTime(booking.pickupDateTime)}</p></DetailCard>
        <DetailCard title="Fare & Payment" icon={<CircleDollarSign size={18}/>}><SummaryLine label="Base Fare" value={money(booking.baseFare??booking.estimatedFare)}/><SummaryLine label="Discount" value={`− ${money(booking.discountAmount)}`}/><SummaryLine label="Extra Charges" value={money(booking.extraCharges)}/><SummaryLine label="Final Fare" value={money(booking.finalFare??booking.estimatedFare)} strong/>{booking.transactions?.[0]&&<><SummaryLine label="Payment" value={statusLabel(booking.transactions[0].paymentMethod)}/><SummaryLine label="Payment Status" value={statusLabel(booking.transactions[0].paymentStatus)}/></>}</DetailCard>
      </div>
      <div className="border-t bg-slate-50 px-6 py-5 flex flex-wrap justify-between gap-3"><div className="flex gap-2"><button onClick={onEdit} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-black text-white"><Edit3 size={16}/> Edit Booking</button><button disabled title="Live tracking is not available in this view" className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-5 py-3 text-sm font-black text-blue-700"><Radio size={16}/> Live Tracking</button></div>{booking.status!=="CANCELLED"&&booking.status!=="TRIP_COMPLETED"&&<button disabled={actionLoading} onClick={onCancel} className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-5 py-3 text-sm font-black text-white"><XCircle size={16}/> Cancel Booking</button>}</div>
    </div>
  </div>
}
...
function EditBookingModal({booking,onClose,onSaved}:{booking:Booking;onClose:()=>void;onSaved:()=>Promise<void>}) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm"><div className="max-h-[94vh] w-full max-w-6xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
    <div className="sticky top-0 z-20 flex items-center justify-between border-b bg-white px-6 py-5"><div><p className="text-xs font-black uppercase tracking-widest text-blue-600">Super Admin · Booking Control Center</p><h2 className="text-2xl font-black text-slate-900">Edit Booking — {booking.bookingNumber}</h2><p className="mt-1 text-xs text-slate-400">Booking reference and historical payment records are protected.</p></div><button onClick={onClose} className="rounded-xl border p-2"><X size={18}/></button></div>
    {loading?<div className="p-16 text-center font-semibold text-slate-500">Loading real customer, assignment and pricing data...</div>:<><div className="grid gap-5 p-6 lg:grid-cols-2">
      ...
      <EditSection title="Vendor → Vehicle → Driver Assignment"><Field label="Approved Vendor"><select value={form.vendorId} onChange={e=>changeVendor(e.target.value)} className="control">{vendors.map(v=><option key={v.id} value={v.id}>{v.label} · {v.mobile||"No mobile"}</option>)}</select></Field><div className="mt-4"><Field label="Eligible Vehicle"><select value={form.vehicleId} onChange={e=>changeVehicle(e.target.value)} className="control"><option value="">Select vehicle</option>{vehicles.map(v=><option key={v.id} value={v.id}>{v.label}</option>)}</select></Field>{vehicle&&<div className="mt-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-600"><b>{vehicle.make} {vehicle.model}{vehicle.variant?` ${vehicle.variant}`:""}</b> · {vehicle.registrationNumber}<br/>{vehicle.category} · {vehicle.seatingCapacity} seats · {vehicle.fuelType} · {vehicle.transmission} · {vehicle.status}</div>}</div><div className="mt-4"><Field label="Active Driver"><select value={form.driverId} onChange={e=>set("driverId",e.target.value)} className="control"><option value="">No driver assigned</option>{drivers.map(d=><option key={d.id} value={d.id}>{d.label} · {d.mobile||"No mobile"}</option>)}</select></Field>{form.driverId&&<p className="mt-2 text-xs font-semibold text-blue-600">{drivers.find(d=>d.id===form.driverId)?.mobile||"Driver phone not available"}</p>}</div></EditSection>
      <EditSection title="Pricing Package (Source of Truth)"><Field label="Active Pricing Package"><select value={form.pricingPackageId} onChange={e=>set("pricingPackageId",e.target.value)} className="control"><option value="">Select pricing package</option>{compatiblePackages.map(p=><option key={p.id} value={p.id}>{p.label} · {p.pricingType} · {p.tripType} · {money(p.baseFare)}</option>)}</select></Field>{pkg?<div className="mt-4 rounded-2xl border bg-slate-50 p-4"><div className="flex justify-between gap-3"><div><p className="font-black">{pkg.label}</p><p className="text-xs text-slate-500">{pkg.pricingType} · {pkg.tripType} · {pkg.chargeType}</p>{pkg.fromCity&&pkg.toCity&&<p className="text-xs text-slate-500">{pkg.fromCity} → {pkg.toCity}</p>}{pkg.airportName&&<p className="text-xs text-slate-500">{pkg.airportName} · {pkg.transferDirection}</p>}</div><p className="text-xl font-black">{money(pkg.baseFare)}</p></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><PricingLine label="Included KM" value={pkg.includedKm==null?"—":String(pkg.includedKm)}/><PricingLine label="Included Hours" value={pkg.includedHours==null?"—":String(pkg.includedHours)}/><PricingLine label="Extra KM" value={money(pkg.extraKmRate)}/><PricingLine label="Extra Hour" value={money(pkg.extraHourRate)}/><PricingLine label="Driver Allowance" value={money(pkg.driverAllowance)}/><PricingLine label="Night" value={money(pkg.nightCharge)}/><PricingLine label="Toll" value={money(pkg.tollCharge)}/><PricingLine label="Parking" value={money(pkg.parkingCharge)}/></div></div>:<p className="mt-3 text-xs text-amber-600">Select an active package. The fare is not manually editable.</p>}</EditSection>
      <EditSection title="Financial Record"><p className="mb-3 text-xs text-slate-500">Existing payment transactions remain untouched. Selecting a new package changes booking fare from the saved package values only.</p><SummaryLine label="Current Base Fare" value={money(booking.baseFare??booking.estimatedFare)}/><SummaryLine label="Discount" value={`− ${money(booking.discountAmount)}`}/><SummaryLine label="Current Extra Charges" value={money(booking.extraCharges)}/><SummaryLine label="Current Final Fare" value={money(booking.finalFare??booking.estimatedFare)} strong/></EditSection>
    </div>{error&&<div className="mx-6 mb-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{error}</div>}<div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 border-t bg-slate-50 px-6 py-5"><p className="text-xs text-slate-500">Validation: approved vendor → verified/available vehicle → active driver → active pricing package.</p><div className="flex gap-3"><button onClick={onClose} className="rounded-xl border bg-white px-5 py-3 text-sm font-bold">Cancel</button><button onClick={()=>void save()} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-sm font-black text-white disabled:opacity-50"><Save size={16}/>{saving?"Saving...":"Save Changes"}</button></div></div></>}</div></div>
}
