  ...
  return {
    ...
    dateOfBirth:
      driver.dateOfBirth instanceof Date
        ? driver.dateOfBirth.toISOString().slice(0, 10)
        : driver.dateOfBirth ?? "",
...
export async function GET(request: NextRequest) {
  try {
    const denied = await centralFleetAccess(request, "drivers"); if (denied) return denied;
...
export async function POST(
  ...
  try {
    ...
    const policeVerificationNumber =
      String(
        body.policeVerificationNumber ?? ""
      ).trim();
    ...
    const driver =
      await prisma.$transaction(
        async (tx) => {
          /*
           * Recreate a previously soft-deleted driver
           * when the same licence and email are supplied.
           */
          const deletedDriver =
...
export async function PUT(
  ...
  try {
    ...
    const policeVerificationNumber =
      body.policeVerificationNumber !== undefined
        ? String(body.policeVerificationNumber).trim()
        : existing.policeVerificationNumber ?? null;
    ...
    if (
      ...
      email.trim().toLowerCase() !==
        (existing.user?.email ?? "").trim().toLowerCase()
    ) {
      const emailOwner =
        await prisma.user.findFirst({
          where: {
