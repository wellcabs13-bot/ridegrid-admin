...
export async function GET(request: NextRequest) {
  try {
    ...
    if (
      !Object.values(AutomationTrigger).includes(
    ...
    return NextResponse.json({
      success: true,
      trigger,
      message: "Automation trigger is valid.",
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !body.context ||
      typeof body.context.module !== "string"
    ) {
      return NextResponse.json(
        {
          success: false,
          message: "context.module is required.",
        },
        { status: 400 }
      );
    }
    ...
    const event = createAutomationEvent({
      ...
      bookingId: body.context.bookingId,
      vendorId: body.context.vendorId,
      driverId: body.context.driverId,
