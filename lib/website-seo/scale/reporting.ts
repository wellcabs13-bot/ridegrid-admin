...
export async function overview() {
  const coverage = await Promise.all(WEBSITE_ENTITY_TYPES.map(async entityType => {
    const pageWhere = { entity: { type: entityType } };
    const [entities, pages, published, ready, blocked, keywords, content, seo, coveredEntities] = await Promise.all([
    ...
    return { entityType, entities, pages, published, ready, blocked, keywords, content, seo, coveragePercent: entities ? Math.round(coveredEntities / entities * 100) : null };
  }));
