...
export interface PublicSeo { title: string; description: string; canonical: string; robots: { index: boolean; follow: boolean }; schema: SeoSchema }
export interface PublicPage { title: string; entityName: string; entityType: string; pathname: string; seo: PublicSeo; sections: PublicSection[]; links: PublicLink[]; breadcrumbs: PublicLink[] }
export interface PublicChrome { navigation: PublicNavLink[]; blocks: PublicBlock[]; media: { category: string; asset: PublicMedia }[] }
